...
| Item | Next task |
|---|---|
| ExercisePaperStyle | T2 — write and compile one scaffolded LaTeX paper under the same guide |

`ExercisePaperStyle` started on 2026-08-20 and its T1 ran the shared writing guide against a real document for the first time: a short paper on equidistance and odd girth, built from the SyntheticInfrageometry paclet's own primitives and proved in full. It found [seven writing rules and three build defects](../Wiki/Concepts/PaperStyleExercise.md), two of the seven being rules of `style.md` that contradict each other, and it left the notebook undeployed pending the operator's read. `JournalAsPaperSink`, filed the same day as it (2026-08-18), stays in `Backlog/`: it resolves the tier rules depending on a journal that is off by default, and T1 gave it one data point — nothing in that session fell below the settled tier, so the gap did not bite.
