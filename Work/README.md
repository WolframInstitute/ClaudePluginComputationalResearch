...
`JournalAsPaperSink`, filed the same day as it (2026-08-18), stays in `Backlog/`: it resolves the tier rules depending on a journal that is off by default, and T1 gave it one data point — nothing in that session fell below the settled tier, so the gap did not bite.
