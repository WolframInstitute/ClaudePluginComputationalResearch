...
| Item | Next task |
|---|---|
| [JournalAsPaperSink](Active/JournalAsPaperSink.md) | T2 — build a paper with the journal off and confirm nothing is silently lost |
...
`JournalAsPaperSink`, filed the same day as `ExercisePaperStyle` (2026-08-18), started on 2026-08-21 and is the one active item: it resolves the tier rules depending on a journal that is off by default.
Its T1 ruled, with the operator, that the generator must [refuse to cut *silently*](../Wiki/Concepts/CutWithNoJournal.md) — stop, list what has no home, and let the operator turn the journal on, keep it marked in the paper, or drop it explicitly, with retain-and-report as the fixed unattended default.
Each of the three filed candidates broke a rule the plugin already had, and the surface turned out to be five sites rather than the three the Spec named; the rule is now written once in `style.md` and linked from the rest.
The same session deleted the two `ExercisePaperStyle` documents, whose findings were already harvested — so T2 must construct a document that genuinely cuts something, since the exercise that was meant to supply one produced nothing below the settled tier.
