...
The paper tiers no longer depend on a toggle being on — see [Cutting with no journal](Concepts/CutWithNoJournal.md).
They route everything marginal to the journal, which is off by default, and the off case had one parenthesis sending it to the Wiki instead: an encyclopedia is the wrong shape for a dated record of what did not make the paper, so the material was either distorted or dropped.
With the journal off the generator now **refuses to cut silently** — it stops, lists what has no home, and the operator turns the journal on, keeps the material marked in the paper, or drops it explicitly; unattended, the choice is fixed to retain-and-report in the run digest.
The rule is written once in `style.md` § *When the journal is off* and linked from the other four sites, one of which (`scaffold-paper`) the Spec had not counted.
Untested as yet: nothing has ever produced a document that cuts anything, which is why the deciding session had zero observations to work from.
...
The interactive half of that fix closed on 2026-08-21 in the paclet: the square is content, so the importer, the palette's *Proof* button and *Continue block* each write or move it, and the **exporter drops it** — without which a paper carrying a square exported its whole `Cell` expression into the `.tex`, caught by the sample round trips going byte-inexact.
...
Both exercise documents failed the rule they produced, and `ExampleEveryDefinition` [closed that the same day](Concepts/PaperStyleExercise.md#exercising-the-corrected-rule) with six examples per path — verified on a kernel and approved from a contact sheet before either document was touched.
It found three things, all in the build path: a highlighted set drawn over a highlighted pair hides it, so a picture has to be looked at and not only computed; the stale graphics fingerprint has a mechanism, which is stamping the in-memory notebook rather than the round-tripped one; and which code fences get an evaluated Output was undocumented, with the obvious implementation silently embedding nothing.
