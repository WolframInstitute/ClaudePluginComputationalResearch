...
Use `--dry-run` first if the user has not run this before — it prints the selection, the branch, the next task, and the caps without spawning anything.
...
3. the `## Hand-off` delta — for `needs-human` this is the question the run refused to guess at, so quote it;
4. for `permission-denied`, the tools it wanted, and that re-running with `--allow '<Tool>(<pattern>)'` is the fix.
