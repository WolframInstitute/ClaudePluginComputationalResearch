...
A typo'd annotation halts the run as `bad-annotation` before anything is spawned, so `--dry-run` is also how you check that a task's routing parses.
...
6. any `**escalate?**` line — a task that halted under a cheap tier, with the annotation to raise it to before re-running. The recommendation is the digest's, and the decision is the user's;
7. `bad-annotation`, which names the field it could not parse and costs nothing: fix the annotation in the item file and re-run.
