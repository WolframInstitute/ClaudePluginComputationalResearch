...
Then check **license headroom** (this is the usual cause of `wolframscript`
failures). On the official MCP — which reuses one persistent kernel and costs
no extra seat — evaluate with `mcp__Wolfram__WolframLanguageEvaluator`:
...
Report `Max / InUse / Free`. If `Free <= 0`, warn that **any `wolframscript`
spawn will fail with a license error** — each running Wolfram MCP server and
each open front-end holds one of the seats, and two Wolfram MCP servers
(official `Wolfram` + unofficial `wolfram`) are commonly configured at once.
Tell the user to either route work through the MCP (preferred — no new seat) or
free a seat (close a front-end, or disable whichever Wolfram MCP server is
unused).
...
6. **License headroom** — `Free` seats, and whether the environment is
   saturated (so the plugin should stay on the MCP path and avoid spawning
   `wolframscript`)
