...
description: >
  ...
  hands them to resource-add. Use when the user says "search math",
...
Companion to [wolfram-resources](../wolfram-resources/SKILL.md) — searches
...
1. If `Wiki/` exists: invoke [resource-add](../resource-add/SKILL.md) with the
   URL — resource-add will recognise the URL pattern and create the right kind
   of wiki article (MathWorld / nLab / OEIS / DLMF / Wikipedia).
2. If no `Wiki/`: just print the URLs as references the user can keep.

## Integration with other skills

- Pair with [wolfram-resources](../wolfram-resources/SKILL.md) when the project
  also touches Wolfram-ecosystem material.
- Pair with [cite-from-id](../cite-from-id/SKILL.md) to immediately produce a
  BibTeX entry for any DOI/arXiv ID surfaced (e.g. references inside a
  MathWorld or Wikipedia article).
- Pair with [resource-add](../resource-add/SKILL.md) to commit kept results
  to `Wiki/Resources/`.

## When NOT to use

- Pure Wolfram-ecosystem questions → use [wolfram-resources](../wolfram-resources/SKILL.md).
- Looking for a specific paper (arXiv / DOI / title) → use the existing arXiv MCP
  and [cite-from-id](../cite-from-id/SKILL.md).
