...
if [ "$DRY_RUN" = 1 ]; then
  ...
  echo "next task : $(next_task "$ITEM_FILE")"
...
while :; do
  ...
  echo "auto-run: $ITEM $TASK_ID — $(date -u +%H:%M:%SZ)"
  ...
  SUMMARY="$TASK_ID — turns $TURNS, \$$(printf '%.4f' "$COST"), subtype \`$SUBTYPE\`"
  ...
  [ "$STOPR" = "end_turn" ] || { emit "- **halt** $SUMMARY, stop_reason \`$STOPR\`"; halt "stop-reason"; }
  ...
  case "${TERMR:-completed}" in
    completed|end_turn) ;;
    *) emit "- **halt** $SUMMARY, terminal_reason \`$TERMR\`"; halt "terminal-reason" ;;
  esac
  ...
  [ "$HEAD_AFTER" != "$HEAD_BEFORE" ] \
    || { emit "- **halt** $SUMMARY — no new commit"; halt "no-commit"; }
  [ "$DONE_AFTER" -gt "$DONE_BEFORE" ] \
    || { emit "- **halt** $SUMMARY — \`### Done\` gained no box ($DONE_BEFORE → $DONE_AFTER)"; halt "no-box"; }
